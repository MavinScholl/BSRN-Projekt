...
int main() {
    ...
    // ProzessID & ElternprozessID auslesen und in die Datei speichern
    char prozessID[10];
    sprintf(prozessID, "%d", getpid());

    char komma[] = ", ";

    char elternprozessID[10];
    sprintf(elternprozessID, "%d", getppid());

    strcat(prozessID, komma);
    strcat(prozessID, elternprozessID);
