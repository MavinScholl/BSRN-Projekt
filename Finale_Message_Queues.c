...
void processOne() {     //der Prozess für Conv
    while (!shouldExit) {
        // Platzhaltercode für Übersicht
        sleep(1);
    }
}

void processTwo() {     //der Prozess für Log
    while (!shouldExit) {
        // Platzhaltercode für Übersicht
        sleep(1);
    }
...
void processThree() {   //der Prozess für Stat
    while (!shouldExit) {
        // Platzhaltercode für Übersicht
        sleep(1);
    }
...
int main() {
    ...
    // Erster paralleler Prozess starten / Conv beginnt
    pid1 = fork();
    if (pid1 == 0) {
        // Kindprozess - Prozess One
        processOne();
        exit(0);
    } else if (pid1 < 0) {
        // Fehler beim Forken
        perror("Fehler beim Starten des ersten parallelen Prozesses");
        return 1;
    }

    // Zweiter paralleler Prozess starten / Log beginnt
    pid2 = fork();
    if (pid2 == 0) {
        // Kindprozess - Prozess Two
        processTwo();
        exit(0);
    } else if (pid2 < 0) {
        // Fehler beim Forken
        perror("Fehler beim Starten des zweiten parallelen Prozesses");
        return 1;
    }

    // Dritter paralleler Prozess starten / Stat beginnt
    pid3 = fork();
    if (pid3 == 0) {
        // Kindprozess - Prozess Three
        processThree();
        exit(0);
    } else if (pid3 < 0) {
        // Fehler beim Forken
        perror("Fehler beim Starten des dritten parallelen Prozesses");
        return 1;
    }
